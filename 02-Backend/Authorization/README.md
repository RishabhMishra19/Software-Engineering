...
Authorization decides whether a principal may perform a specific action on a specific resource in the current context. It follows [authentication](../Authentication/README.md) but must not rely solely on a client-provided role or hidden user-interface control.
...
- **RBAC:** roles group permissions. It is understandable and efficient but can cause role explosion when context and object ownership vary.
- **ABAC:** policies evaluate principal, resource, action, and environment attributes. It is expressive but harder to reason about and test.
...
- **ACLs:** resources list principals or groups and allowed operations; direct but expensive to manage at very large scale.
...
6. **Why can roles in JWTs become unsafe?** The claims remain stale until token expiry unless introspection, short lifetimes, or revocation mechanisms intervene.
